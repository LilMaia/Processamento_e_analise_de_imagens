...
def resetar_zoom(image_label, image_info, result_label):
    ...
    atualizar_imagem(image_info.image_resized,
                     image_label, image_info, result_label)
...
def aumentar_zoom(image_label, image_info, result_label):
    ...
    if new_zoom_level <= zoom_max:
        ...
        zoom_width = int(image_info.image_resized.width *
                         (1 + new_zoom_level/10))
        zoom_height = int(image_info.image_resized.height *
                          (1 + new_zoom_level/10))
        ...
        image_info.image_resized = image_info.image_resized.resize((zoom_width, zoom_height), Image.LANCZOS).crop(
            (x_original, y_original, x_original + image_info.image_resized.width, y_original + image_info.image_resized.height)).resize((400, 400), Image.LANCZOS)
